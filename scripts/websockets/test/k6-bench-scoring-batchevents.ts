...
// For perftest
npm run build && k6 run dist/k6-bench-scoring.js --log-output stdout -e TENANT=ca -e ENVIRONMENT=perftest -e TESTING=1
npm run build && k6 k6 run dist/k6-bench-scoring-batchevents.js -e TENANT=ca -e ENVIRONMENT=perftest --stage 1m:1,18m:1,1m:0 --out influxdb=https://influxdb.k6.factory.com/livescoring_ca_v3_s63 --logformat=raw --console-output=.\scripts\k6\live-scoring-ca\data\scoringBatchevents.csv
...
const remove_events = new Counter("remove-events");
const batch_count = new Counter("batch-count");
...
export let options = __ENV.TESTING ? { iterations: 1, vus: 1 } : {
    ...setup_ramp(["10"], ["1m"], "live-scoring-ca"), thresholds: {
    ...
    const res = ws.connect(url, params, function (socket) {
        socket.on("open", () => {
            ...
            */
            ...
            let tatalBallCounter = 0;
            let currentBall = 0
            let batchSize = randomIntBetween(5, 15);
            let batchCount = 0;
            let eventStack: GameEvents[] = []

            socket.setInterval(function timeout() {
                //batch event firing will pause after specified  batch count
                if (batchCount >= 10) {
                    //fail('batch count exceeded');
                    socket.close();
                    //exec.test.abort('batch count exceeded');
                    //clearInterval(res);
                ...
                currentBall++
                tatalBallCounter++;
                //remove event and reset over counter
                if (tatalBallCounter % 15 == 0) {
                    const removeEvent = getRemoveEvent(ballEvent.id);
                    eventStack.push(removeEvent)
                    //console.log(`${JSON.stringify({ "uuid": removeEvent.id, "timestamp": Date.now(), "eventType": "removeevent", "VU": __VU, "gameId": gameID })}`);
                    remove_events.add(1);
                    currentBall--;
                } else {
                ...
                if (eventStack.length >= batchSize) {
                    ...
                    for (let i = 0; i < eventStack.length; i += batchSize) {
                        ...
                        //console.log("Sending batch", chunk.length)
                        console.log(`VU ${me}, Sending batch No ${batchCount + 1}, Batch Size ${chunk.length}`);
                        //console.log("Sending event batch ==> ", JSON.stringify(chunk));
                        socket.send(JSON.stringify(getEventProcessPayload(chunk)
                        ))
                    }
                    //  batchSize++
                    batch_count.add(1);
                    batchSize = randomIntBetween(7, 12);
                    batchCount++;
                    eventStack = []
                    //interval between two batches. This will simulate bunch of events scored offline and triggers suddenly. 
                    sleep(randomIntBetween(240, 360));
                }
